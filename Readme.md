...
# Комментарии.
    ...
    4) комментарии к методу doOperation() и к классу не отражают реальность. Будут добавлены
    5) doOperation() возвращает массив, а не void. Будет исправлено
    6) такой способ получения данных 
    ``...$data = (array)$this->getRequest('data')...``
    вероятно не правильный,  здесь можно получить лишние данные, лучше передать их как входные параметры или они должны быть ранее получены в private переменную
